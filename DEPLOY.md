...
**Причина:** Файл `config.js` отсутствует или не содержит API ключей.

**Решение: Создать config.js перед деплоем**
...
# 3. Добавьте config.js в Git для деплоя
git add js/config.js
git commit -m "Add config.js with API keys"
git push

# После деплоя приложение будет работать с вашими ключами
```
...
**Важно:** 
- Файл `config.js` содержит ваши API ключи и будет в репозитории после деплоя
- Если вы используете публичный репозиторий, рассмотрите использование приватного репозитория или используйте Serverless Functions для безопасного хранения ключей
